...
// SelectionBorders
SelectionBorders::SelectionBorders(Window *window)
    : IHyprWindowDecoration(window->get_window()), window(window) {
...
void SelectionBorders::onPositioningReply(
...
void SelectionBorders::draw(PHLMONITOR pMonitor, float const &a) {
  ...
  g_pHyprRenderer->m_sRenderPass.add(
      Hyprutils::Memory::makeUnique<CBorderPassElement>(data));
}
...
void JumpDecoration::onPositioningReply(
    const SDecorationPositioningReply &reply) {
  m_bAssignedGeometry = reply.assignedGeometry;
}

CBox JumpDecoration::assignedBoxGlobal() {
  static auto *const *TEXTSCALE =
      (Hyprlang::FLOAT *const *)HyprlandAPI::getConfigValue(
          PHANDLE, "plugin:scroller:jump_labels_scale")
          ->getDataStaticPtr();
  CBox box = m_bAssignedGeometry;
  box.translate(g_pDecorationPositioner->getEdgeDefinedPoint(
      DECORATION_EDGE_BOTTOM | DECORATION_EDGE_LEFT | DECORATION_EDGE_RIGHT |
  ...
  const double scale = **TEXTSCALE < 0.1   ? 0.1
                       : **TEXTSCALE > 1.0 ? 1.0
                                           : **TEXTSCALE;
...
void JumpDecoration::draw(PHLMONITOR pMonitor, float const &a) {
  ...
  if (m_pTexture.get() == nullptr) {
    ...
    static auto const *TEXTFONTFAMILY =
        (Hyprlang::STRING const *)HyprlandAPI::getConfigValue(
            PHANDLE, "plugin:scroller:jump_labels_font")
            ->getDataStaticPtr();
    static auto *const *TEXTCOL =
        (Hyprlang::INT *const *)HyprlandAPI::getConfigValue(
            PHANDLE, "plugin:scroller:jump_labels_color")
            ->getDataStaticPtr();
  ...
  data.box = windowBox;
  g_pHyprRenderer->m_sRenderPass.add(
      Hyprutils::Memory::makeUnique<CTexPassElement>(data));
}
...
std::string JumpDecoration::getDisplayName() { return "Overview"; }
