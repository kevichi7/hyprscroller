...
public:
  ...
  virtual const char *passName() { return "OverviewPassElement"; }
...
static bool hookVisibleOnMonitor(void *thisptr, PHLMONITOR monitor) {
  ...
  return ((origVisibleOnMonitor)(g_pVisibleOnMonitorHook->m_pOriginal))(
      thisptr, monitor);
}

// Needed to undo the monitor scale to render layers at the original scale
static void hookRenderLayer(void *thisptr, PHLLS layer, PHLMONITOR monitor,
                            timespec *time, bool popups) {
  ...
  if (overview_enabled) {
    ...
    ;
    modif_data.modifs.push_back(
        {SRenderModifData::eRenderModifType::RMOD_TYPE_SCALE, scaling});
    modif_data.enabled = true;
    g_pHyprRenderer->m_sRenderPass.add(
        Hyprutils::Memory::makeUnique<OverviewPassElement>(
            OverviewPassElement::OverviewModifData(modif_data)));
    g_pHyprRenderer->damageMonitor(monitor);
  }
  ((origRenderLayer)(g_pRenderLayerHook->m_pOriginal))(thisptr, layer, monitor,
                                                       time, popups);
  if (overview_enabled) {
    g_pHyprRenderer->m_sRenderPass.add(
        Hyprutils::Memory::makeUnique<OverviewPassElement>(
            OverviewPassElement::OverviewModifData(SRenderModifData())));
    monitor->m_scale = scale;
    g_pHyprRenderer->damageMonitor(monitor);
  }
}

// Needed to scale the range of the cursor in overview mode to cover the whole
// area.
static CBox hookLogicalBox(void *thisptr) {
  ...
  if (overview_enabled) {
    monitor->vecSize =
        overviews->get_vecsize(workspace) / overviews->get_scale(workspace);
  }
...
static Vector2D hookGetCursorPosForMonitor(void *thisptr, PHLMONITOR monitor) {
  ...
  Vector2D pos = ((
      origGetCursorPosForMonitor)(g_pGetCursorPosForMonitorHook->m_pOriginal))(
      thisptr, monitor);
  if (overview_enabled) {
...
static void hookRenderSoftwareCursorsFor(void *thisptr, PHLMONITOR monitor,
                                         timespec *now, CRegion &damage,
                                         std::optional<Vector2D> overridePos) {
  // Should render the cursor for all the extent of the workspace, and only on
  // overview workspaces when there is one active, and it is in the current
  // monitor.
  ...
  if (monitor == last) {
    ...
    if (overviews->overview_enabled(workspace)) {
      ...
      monitor->vecSize =
          overviews->get_vecsize(workspace) / overviews->get_scale(workspace);
      ((origRenderSoftwareCursorsFor)(g_pRenderSoftwareCursorsForHook
                                          ->m_pOriginal))(thisptr, monitor, now,
                                                          damage, overridePos);
      monitor->vecSize = monitor_size;
    }
...
static PHLMONITOR hookGetMonitorFromVector(void *thisptr,
                                           const Vector2D &point) {
  ...
  for (auto const &m : compositor->m_vMonitors) {
    ...
    if (!workspace)
      workspace = m->activeWorkspaceID();
    bool overview_enabled = overviews->overview_enabled(workspace);
    Vector2D vecSize = overview_enabled ? overviews->get_vecsize(workspace) /
                                              overviews->get_scale(workspace)
                                        : m->vecSize;
  ...
  for (auto const &m : compositor->m_vMonitors) {
    float dist = vecToRectDistanceSquared(point, m->vecPosition,
                                          m->vecPosition + m->vecSize);
...
Overview::Overview() : initialized(false) {
  ...
  if (!FNS1.empty()) {
    g_pVisibleOnMonitorHook = HyprlandAPI::createFunctionHook(
        PHANDLE, FNS1[0].address, (bool *)hookVisibleOnMonitor);
  ...
  if (!FNS3.empty()) {
    g_pRenderLayerHook = HyprlandAPI::createFunctionHook(
        PHANDLE, FNS3[0].address, (bool *)hookRenderLayer);
  ...
  if (!FNS5.empty()) {
    g_pLogicalBoxHook = HyprlandAPI::createFunctionHook(
        PHANDLE, FNS5[0].address, (bool *)hookLogicalBox);
  ...
  auto FNS6 =
      HyprlandAPI::findFunctionsByName(PHANDLE, "renderSoftwareCursorsFor");
  if (!FNS5.empty()) {
    g_pRenderSoftwareCursorsForHook = HyprlandAPI::createFunctionHook(
        PHANDLE, FNS6[0].address, (bool *)hookRenderSoftwareCursorsFor);
  ...
  auto FNS7 = HyprlandAPI::findFunctionsByName(PHANDLE, "getMonitorFromVector");
  if (!FNS5.empty()) {
    g_pGetMonitorFromVectorHook = HyprlandAPI::createFunctionHook(
        PHANDLE, FNS7[0].address, (bool *)hookGetMonitorFromVector);
  ...
  if (!FNS5.empty()) {
    g_pClosestValidHook = HyprlandAPI::createFunctionHook(
        PHANDLE, FNS8[0].address, (bool *)hookClosestValid);
  ...
  if (!FNS2.empty()) {
    g_pRenderMonitorHook = HyprlandAPI::createFunctionHook(
        PHANDLE, FNS2[0].address, (bool *)hookRenderMonitor);
  ...
  auto FNS4 =
      HyprlandAPI::findFunctionsByName(PHANDLE, "getCursorPosForMonitor");
  if (!FNS4.empty()) {
    g_pGetCursorPosForMonitorHook = HyprlandAPI::createFunctionHook(
        PHANDLE, FNS4[0].address, (bool *)hookGetCursorPosForMonitor);
...
Overview::~Overview() {
  ...
  if (g_pGetCursorPosForMonitorHook != nullptr) {
    /* bool success = */ HyprlandAPI::removeFunctionHook(
        PHANDLE, g_pGetCursorPosForMonitorHook);
    g_pGetCursorPosForMonitorHook = nullptr;
  }

  if (g_pRenderMonitorHook != nullptr) {
    /* bool success = */ HyprlandAPI::removeFunctionHook(PHANDLE,
  ...
  if (g_pGetMonitorFromVectorHook != nullptr) {
    /* bool success = */ HyprlandAPI::removeFunctionHook(
        PHANDLE, g_pGetMonitorFromVectorHook);
    g_pGetMonitorFromVectorHook = nullptr;
  }

  if (g_pRenderSoftwareCursorsForHook != nullptr) {
    /* bool success = */ HyprlandAPI::removeFunctionHook(
        PHANDLE, g_pRenderSoftwareCursorsForHook);
  ...
  if (g_pVisibleOnMonitorHook != nullptr) {
    /* bool success = */ HyprlandAPI::removeFunctionHook(
        PHANDLE, g_pVisibleOnMonitorHook);
...
void Overview::disable(WORKSPACEID workspace) {
  ...
  if (!cleared && workspaces.count(workspace)) {
    workspaces[workspace].overview = false;
  }
  if (!overview_enabled())
    disable_hooks();
...
bool Overview::enable_hooks() {
  if (initialized && g_pGetCursorPosForMonitorHook != nullptr &&
      g_pGetCursorPosForMonitorHook->hook() &&
      ...
      g_pGetMonitorFromVectorHook != nullptr &&
      g_pGetMonitorFromVectorHook->hook()) {
    return true;
  }
  return false;
}
...
void Overview::disable_hooks() {
  ...
  if (g_pGetCursorPosForMonitorHook != nullptr)
    g_pGetCursorPosForMonitorHook->unhook();
  if (g_pRenderMonitorHook != nullptr)
  ...
  if (g_pLogicalBoxHook != nullptr)
    g_pLogicalBoxHook->unhook();
  if (g_pRenderSoftwareCursorsForHook != nullptr)
    g_pRenderSoftwareCursorsForHook->unhook();
  if (g_pClosestValidHook != nullptr)
    g_pClosestValidHook->unhook();
  if (g_pGetMonitorFromVectorHook != nullptr)
    g_pGetMonitorFromVectorHook->unhook();
}
