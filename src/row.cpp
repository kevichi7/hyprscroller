...
Row::Row(WORKSPACEID workspace)
    : workspace(workspace), overview(false), reorder(Reorder::Auto),
      pinned(nullptr), active(nullptr) {
  post_event("overview");
...
void Row::find_auto_insert_point(Mode &new_mode,
                                 ListNode<Column *> *&new_active) {
...
void Row::move_focus_begin() { active = columns.first(); }

void Row::move_focus_end() { active = columns.last(); }
...
void Row::resize_active_column(int step) {
  ...
  if (mode == Mode::Column) {
    active->data()->cycle_size_active_window(step, calculate_gap_x(active),
                                             gap);
  } else {
    ...
    if (width == StandardSize::Free) {
      ...
      static auto *const *CYCLESIZE_CLOSEST =
          (Hyprlang::INT *const *)HyprlandAPI::getConfigValue(
              PHANDLE, "plugin:scroller:cyclesize_closest")
              ->getDataStaticPtr();
      if (**CYCLESIZE_CLOSEST) {
        double fraction = active->data()->get_geom_w() / max.w;
        width = scroller_sizes.get_column_closest_width(
            g_pCompositor->m_pLastMonitor, fraction, step);
      } else {
...
void Row::size_active_column(const std::string &fraction) {
  ...
  if (std::isdigit(fraction.front())) {
    ...
    size = mode == Mode::Row ? size = scroller_sizes.get_column_width(index)
                             : size = scroller_sizes.get_window_height(index);
  } else {
    StandardSize default_size = mode == Mode::Row
                                    ? scroller_sizes.get_column_width(0)
                                    : scroller_sizes.get_window_height(0);
...
Mode Row::get_mode() const { return mode; }
...
ModeModifier Row::get_mode_modifier() const { return modifier; }
...
void Row::selection_toggle() { active->data()->selection_toggle(); }
...
static void insert_selection_before(List<Column *> &columns,
                                    ListNode<Column *> *node,
                                    const List<Column *> &selection) {
  for (auto col = selection.first(); col != nullptr; col = col->next()) {
    columns.insert_before(node, col->data());
  }
}

static void insert_selection_after(List<Column *> &columns,
                                   ListNode<Column *> *node,
                                   const List<Column *> &selection) {
...
void Row::expel_window(AdmitExpelDirection dir) {
  ...
  // will be the same, and on the left there will be a gap (to the column it
  // left)
  Vector2D gap_x = calculate_gap_x(active);
  if (dir == AdmitExpelDirection::Left)
    gap_x.y = gap;
  ...
  if (dir == AdmitExpelDirection::Left) {
    ...
    active->data()->set_geom_pos(active->next()->data()->get_geom_x() -
...
void Row::post_event(const std::string &event) {
  if (event == "mode") {
    ...
    g_pEventManager->postEvent(SHyprIPCEvent{
        "scroller",
        std::format("mode, {}, {}, {}, {}:{}, {}, {}", str_mode,
                    modifier.get_position_string(), modifier.get_focus_string(),
                    modifier.get_auto_mode_string(), modifier.get_auto_param(),
                    modifier.get_center_column_string(),
                    modifier.get_center_window_string())});
  } else if (event == "overview") {
    g_pEventManager->postEvent(SHyprIPCEvent{
        "scroller", std::format("overview, {}", overview ? 1 : 0)});
...
bool Row::update_sizes(PHLMONITOR monitor) {
  ...
  // g_pConfigManager->getWorkspaceRuleFor(g_pCompositor->getWorkspaceByID(workspace));
  ...
  auto gaps_in = PGAPSIN ? PGAPSIN->m_top : 0;
  auto gaps_out = PGAPSOUT ? *PGAPSOUT : 0;
  ...
  full = Box(POS, SIZE);
  const Box newmax = Box(
      POS.x + TOPLEFT.x + gaps_out.m_left, POS.y + TOPLEFT.y + gaps_out.m_top,
...
void Row::set_fullscreen_mode(PHLWINDOW window, eFullscreenMode cur_mode,
...
bool Row::is_overview() const { return overview; }
  ...
  if (overview) {
    ...
    if (overview_scaled) {
      ...
      for (auto c = columns.first(); c != nullptr; c = c->next()) {
        ...
        col->set_geom_pos(full.x + (max.x - full.x + offset.x) / scale +
                              (col->get_geom_x() - bmin.x),
                          full.y + (max.y - full.y + offset.y) / scale +
                              (cheight.x - bmin.y));
      }
      ...
      for (auto c = columns.first(); c != nullptr; c = c->next()) {
        ...
        col->push_overview_geom();
        Vector2D cheight = col->get_height();
        col->set_geom_pos(offset.x + max.x +
                              (col->get_geom_x() - bmin.x) * scale,
                          offset.y + max.y + (cheight.x - bmin.y) * scale);
...
void Row::update_windows(const Box &oldmax, bool force) {
  ...
  if (active && oldmax != max) {
    double posx =
        max.x + max.w * (active->data()->get_geom_x() - oldmax.x) / oldmax.w;
    double posy =
        max.y + max.h * (active->data()->get_geom_vy() - oldmax.y) / oldmax.h;
...
void Row::recalculate_row_geometry() {
  ...
  static auto *const *center_row =
      (Hyprlang::INT *const *)HyprlandAPI::getConfigValue(
          PHANDLE, "plugin:scroller:center_row_if_space_available")
          ->getDataStaticPtr();
  ...
  if (pinned != nullptr) {
    ...
    if (a_x < max.x || std::round(a_x + a_w) > max.x + max.w) {
      ...
      if (pinned != active) {
        int p = 0, a = 0;
        int i = 0;
        for (auto col = columns.first(); col != nullptr;
             col = col->next(), ++i) {
          if (col == pinned)
            p = i;
          if (col == active)
  ...
  if (a_x < max.x) {
    ...
    if (reorder == Reorder::Auto) {
      ...
      if (active->prev() != nullptr) {
        ...
        if (prev->get_geom_x() >= max.x &&
            std::round(prev->get_geom_x() + prev->get_geom_w()) <=
                max.x + max.w) {
          keep_current = true;
        }
      }
      if (!keep_current && active->next() != nullptr) {
        ...
        if (next->get_geom_x() >= max.x &&
            std::round(next->get_geom_x() + next->get_geom_w()) <=
                max.x + max.w) {
          keep_current = true;
        }
      }
        ...
        if (active->next() != nullptr) {
          if (std::round(a_w + active->next()->data()->get_geom_w()) <= max.w) {
            ...
            active->data()->set_geom_pos(
                max.x + max.w - a_w - active->next()->data()->get_geom_w(),
                max.y);
          } else if (active->prev() != nullptr) {
            if (std::round(active->prev()->data()->get_geom_w() + a_w) <=
                max.w) {
              // set previous at the left edge of the viewport
              active->data()->set_geom_pos(
                  max.x + active->prev()->data()->get_geom_w(), max.y);
            } else {
          ...
          if (std::round(active->prev()->data()->get_geom_w() + a_w) <= max.w) {
            ...
            // it doesn't fit and nothing on the right, move active to right
            // edge of viewport
            active->data()->set_geom_pos(max.x + max.w - a_w, max.y);
          }
        } else {
...
void Row::adjust_columns(ListNode<Column *> *column) {
  ...
  for (auto col = column->prev(), prev = column; col != nullptr;
       prev = col, col = col->prev()) {
    col->data()->set_geom_pos(
        prev->data()->get_geom_x() - col->data()->get_geom_w(), max.y);
  }
  // Adjust the positions of the columns to the right
  for (auto col = column->next(), prev = column; col != nullptr;
       prev = col, col = col->next()) {
    col->data()->set_geom_pos(
        prev->data()->get_geom_x() + prev->data()->get_geom_w(), max.y);
  }
...
void Row::scroll_end(Direction dir) {
  if (dir == Direction::Left) {
    ...
    // This column should be the same while swiping. Mouse coordinates don't
    // change while swiping
    auto column = get_mouse_column();
